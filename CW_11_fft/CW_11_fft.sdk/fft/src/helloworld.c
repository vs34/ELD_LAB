...
const float complex W8[4] = {1 - 0 * I,
...
void FFT_stages(float complex FFT_input[N], float complex FFT_output[N]) {
  ...
  for (int i = 0; i < N; i = i + 4) {
    for (int j = 0; j < 2; ++j) {
      temp2[i + j] = temp1[i + j] + W8[2 * j] * temp1[i + j + 2];
      temp2[i + j + 2] = temp1[i + j] - W8[2 * j] * temp1[i + j + 2];
    }
  }
  ...
  for (int i = 0; i < N; i = i + 8) {
    for (int j = 0; j < 4; ++j) {
      temp3[i + j] = temp2[i + j] + W8[j] * temp2[i + j + 4];
      temp3[i + j + 4] = temp2[i + j] - W8[j] * temp2[i + j + 4];
    }
  }

  // Stage 4
  for (int i = 0; i < N / 2; i = i + 1) {
    FFT_output[i] = temp3[i] + W8[i] * temp3[i + 4];
    FFT_output[i + 4] = temp3[i] - W8[i] * temp3[i + 4];
  }

  for (int i = 0;i<N;i++){
	  FFT_output[i] = 1/FFT_output[i]
  }
}
